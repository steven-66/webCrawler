...
public class AddBookUrlThread implements Runnable {
	private String url;
	private parsingServiceImpl content = new parsingServiceImpl();
	//爬取全部小说的网址，书名
	...
	public void run() {
		...
		while(matcher.find()) {
			...
			//并发处理单个书籍信息读取线程
			HttpPoolUtil.executorServiceBook.execute(new AdBookChaptersThread(book));
		}
