...
import parsingService.parsingServiceImpl;
...
public class Main {
	...
	public static void GetAll(String urlFirst) throws Exception {
		parsingServiceImpl content = new parsingServiceImpl();
		...
		if (pageTotalString != null) {
			...
			for (int i = 1; i <= pageTotal; i++) {
				...
				//加入线程池并发处理
				HttpPoolUtil.executorServicePage.execute(new webCrawlerService.AddBookUrlThread(url));
			}
		}
	}
	public static void main(String[] args) throws Exception {
		...
		GetAll(url);
		ExecutorService executorServicePage = HttpPoolUtil.executorServicePage;
		ExecutorService executorServiceBook = HttpPoolUtil.executorServiceBook;
